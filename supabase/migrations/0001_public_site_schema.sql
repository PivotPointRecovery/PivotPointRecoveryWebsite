...
-- Stripe, promoted to 'completed' by the webhook. Abandoned checkouts stay
...
-- The webhook upserts on this, so it has to be unique. Partial index keeps the
-- constraint from tripping over any pre-existing rows with a null session id.
