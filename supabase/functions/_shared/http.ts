...
//
// An explicit origin allowlist, echoing back only a matching origin -- rather
// than the `*` an earlier deployment answered with, which let any site on the
// internet post to these endpoints.
...
function isAllowed(origin: string): boolean {
  ...
  // Cloudflare Pages preview deploys and local dev servers -- non-production
  // surfaces we still want to be able to exercise the forms from.
  try {
