...
// send, so vendoring this into the website repo does not require touching the
// form markup. Saves with the service role, then notifies staff via Resend.
//
...
// mail failure is visible rather than invisible -- the exact condition that
// left earlier volunteer signups unannounced.
  ...
  if (formType === 'volunteer') {
    ...
    const result = await sendNotification(
      ...
      ],
      {
        ...
        intro: `${record.first_name} ${record.last_name} wants to volunteer.`,
      },
    );
